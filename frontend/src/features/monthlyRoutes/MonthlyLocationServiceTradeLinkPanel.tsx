...
export function MonthlyLocationServiceTradeHeroActions({
  location,
  onEditLink,
}: {
  location: LibraryLocation
  ...
  const linkedId = location.service_trade_site_location_id ?? null
  const linkedUrl = location.service_trade_site_location_url ?? null
  if (linkedId == null || !linkedUrl) {
    return null
  }

  return (
    <div className="monthly-location-hero-st-link-row">
      <Button
        as="a"
        href={linkedUrl}
        target="_blank"
        ...
        size="sm"
        className="monthly-location-detail-action monthly-location-hero-st-link-btn"
      >
