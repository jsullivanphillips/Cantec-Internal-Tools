"""Backfill monitoring_account_number, monitoring_password, and monitoring_company_id from legacy monitoring_notes."""
...
def _backfill_row(
    ...
    password: str | None,
    company_id: int | None,
) -> tuple[str | None, str | None, str | None, int | None, bool]:
    ...
    if not (next_password or "").strip() and parsed.password:
        next_password = parsed.password.strip()
        changed = True
    ...
    if notes and (parsed.acct or parsed.company or parsed.password):
        rebuilt = rebuild_monitoring_notes(parsed)
        if rebuilt != (notes or "").strip():
            next_notes = rebuilt
            changed = True

    return next_notes, next_account, next_password, next_company_id, changed
...
def run(*, execute: bool) -> dict[str, int]:
    ...
    for ts in MonthlyTestingSite.query.order_by(MonthlyTestingSite.id.asc()).all():
        ...
        notes, account, password, company_id, changed = _backfill_row(
            ts.monitoring_notes,
            ts.monitoring_account_number,
            ts.monitoring_password,
        ...
        if execute:
            ...
            ts.monitoring_password = password
    ...
    for row in MonthlyTestingSiteMonth.query.order_by(MonthlyTestingSiteMonth.id.asc()).all():
        ...
        if execute:
            ...
            row.monitoring_password = password
