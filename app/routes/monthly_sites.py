...
def _augment_library_location_row(row: dict[str, object]) -> dict[str, object]:
    lid = row.get("id")
    if not isinstance(lid, int):
        return row
    loc = (
        MonthlyRouteLocation.query.options(
            joinedload(MonthlyRouteLocation.monthly_site).selectinload(MonthlySite.testing_sites).joinedload(
                MonthlyTestingSite.linked_key
            ),
        )
        .filter_by(id=lid)
        .one_or_none()
    ...
    if loc is None:
        return row
    if loc.monthly_site is None or not loc.monthly_site.testing_sites:
        sync_testing_sites_from_legacy(loc)
        db.session.flush()

    ms = loc.monthly_site
    if ms is None:
        row["monthly_site_id"] = None
...
def monthly_sites_library():
    ...
    for row in data.get("locations", []):
        ...
        if not isinstance(lid, int):
            continue
        loc = MonthlyRouteLocation.query.options(joinedload(MonthlyRouteLocation.monthly_site)).get(lid)
        if loc is None:
            continue
        if loc.monthly_site is None or not loc.monthly_site.testing_sites:
            sync_testing_sites_from_legacy(loc)
    ...
    data["locations"] = [_augment_library_location_row(dict(r)) for r in data.get("locations", [])]
    return jsonify(data)
...
def monthly_sites_library_detail(location_id: int):
    ...
    body["location"] = _augment_library_location_row(loc_dict)
    return jsonify(body)
...
def monthly_sites_create():
    ...
    data["location"] = _augment_library_location_row(loc_dict)
...
def monthly_sites_patch_location(location_id: int):
    ...
    return update_monthly_route_location(location_id)
...
def _apply_testing_site_payload(ts: MonthlyTestingSite, payload: dict) -> tuple[dict[str, str], int] | None:
    ...
    if "ring_detail" in payload:
        ts.ring_detail = (str(payload.get("ring_detail")).strip() or None) if payload.get("ring_detail") else None
    if "facp_detail" in payload:
        ts.facp_detail = (str(payload.get("facp_detail")).strip() or None) if payload.get("facp_detail") else None
