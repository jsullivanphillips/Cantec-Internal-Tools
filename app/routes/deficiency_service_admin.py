...
from app.deficiency.service_eligibility import classify_all_deficiencies, normalize_phrase
...
def list_non_quoteable_phrases():
    ...
    return jsonify({"phrases": [_phrase_to_dict(row) for row in rows]})
...
def create_non_quoteable_phrase():
    ...
    return jsonify({"phrase": _phrase_to_dict(row)}), 201
...
def update_non_quoteable_phrase(phrase_id: int):
    ...
    db.session.commit()
    classify_all_deficiencies()
    return jsonify({"phrase": _phrase_to_dict(row)})
...
def delete_non_quoteable_phrase(phrase_id: int):
    ...
    db.session.commit()
    classify_all_deficiencies()
    return jsonify({"deleted": phrase_id})
