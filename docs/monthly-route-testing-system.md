...
**Annual schedule check (office prep + location detail):** The UI calls ``GET /api/monthly_routes/routes/<id>/runs/annual_schedule_check?month_date=YYYY-MM-01`` (HTTP response cached 1 hour). Prep uses DB cache when ``st_annual_synced_at`` exists; location detail uses the upcoming test month with ``&sync=1`` for the testing-history grid. The location detail hero **Annual** field uses ``GET /api/monthly_routes/library/<id>/saved_annual_month?sync=1`` — a per-site ServiceTrade lookup across a rolling ~24-month window (not limited to the current calendar month). Live job discovery uses ``scheduledDateFrom/To`` with ``status=scheduled,completed`` (``scheduleDateFrom/To`` alone often returns no site-level inspection jobs).
