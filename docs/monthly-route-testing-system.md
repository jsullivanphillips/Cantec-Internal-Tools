...
| `MonthlyLocation` | `monthly_location` | **Library master** — one row per stop: address + PMC + `label`; route assignment; keys; `access_instructions` (site access notes separate from physical key); inspection/monitoring fields; `price_per_month`; `pricing_updated`; `billing_comments`; optional `service_trade_site_location_id` (ST building id; may be shared across rows) |
...
| `MonthlyRouteRunTimingMonth` | `monthly_route_run_timing_month` | Per-route per-month run start/end from ST testing-job onsite clocks (sync script) |
...
| Routes | `GET /api/monthly_routes/routes`, `GET /api/monthly_routes/dashboard`, `GET /api/monthly_routes/dashboard/issues`, `GET /api/monthly_routes/dashboard/route_breakdown`, `GET /api/monthly_routes/dashboard/route_performance`, `PATCH .../routes/<id>`, `GET .../routes/<id>`, `GET .../routes/<id>/key_audit`, `GET .../routes/<id>/performance_breakdown?month_date=` |
...
**Office skip run** (`POST /api/monthly_routes/routes/<id>/runs/skip?month=YYYY-MM-01`): JSON body requires ``skip_category`` (portal skip category, excluding annual) and ``skip_note`` (free-text reason). Allowed only when no ``MonthlyRouteRun`` and no attributed history exist for that route/month (or an existing draft/prepared run in office prep phase), and the month is within the Pacific current month + 1 window. Creates ``source=office_skip``, marks every active library location ``result_status=skipped`` with the supplied category/note on each stop-month row and ``billing_status=do_not_bill``, materializes worksheet stop-month rows, closes the run (field ended + office review + completed), and captures a field submission snapshot. Workflow stage serializes as **Skipped** (distinct from **Completed**). Portal treats ``office_skip`` like ``csv_import`` (read-only).

**Route detail Runs card** (`MonthlyRouteDetailPage`): year toolbar; rows from January through current Pacific month + 1. Months with no run file show **No data** plus **Skip** and **Upload CSV**; months with a draft or prepared run show **Open paperwork** and **Skip**; months past prep show **Open paperwork** only.
...
| `app/scripts/update_monthly_route_run_timing.py` | Sync `monthly_route_run_timing_month` from ST testing jobs: **completed** jobs supply onsite clock timing; **scheduled** jobs (appointment in the Pacific month, released or not) are cached with `sync_status=scheduled` so office can open the ST job before field completion. Stores ``service_trade_job_status`` and ``service_trade_appointment_released`` for the monthlies dashboard route-card dot. Default window: **24** Pacific months lookback (through current month) + **12** months lookahead (`MONTHLY_ROUTE_RUN_TIMING_LOOKBACK` / `MONTHLY_ROUTE_RUN_TIMING_LOOKAHEAD`). |
...
2. **Four-column grid** — **Location** (building name when set, street + city/province/postal, “Open ServiceTrade location” link when linked); **Property management** (company, key, **Annual** from ServiceTrade sync); **Route** (link, next test, last test outcome from `months`); **Billing** (monthly price, startup date).
3. **Tags row** — editable pills at the bottom (see §8.3).

Identity fields (label, building name, address, property management) are edited via **Actions → Edit** on the hero. **Annual** is read-only in the hero (ServiceTrade sync on page load). Page body sections (**Technician details**, **Billing**, **Testing history**, **Comments**) are collapsible accordions (collapsed by default). Technician details uses cards: Access + Monitoring + Panel (row 1); Testing procedures + Tech location comments (row 2).
...
```bash
python -m app.scripts.upload_monthly_sheet --history-only --months-year 2026 --commit --csv-path "app/MASTER MONTHLY SHEET 2026.csv"
```

Rows are stamped ``history_source=master_sheet``. Existing run CSV or portal outcomes (``run_id``, ``test_outcome``, or ``history_source`` of ``route_csv`` / ``technician_portal``) are left unchanged. History-only mode matches the sheet ADDRESS column to library ``label`` (and PMC) when the strict address + PMC + NOTES identity does not match.
...
| File | Role |
|------|------|
| `frontend/src/pages/MonthlyHomePage.tsx` | Monthlies dashboard (`/monthlies`) — header + tabbed shell (**Routes**, **Metrics**, **Tickets**, **Issues**) |
...
| `frontend/src/pages/MonthlyRouteDetailPage.tsx` | Route detail; **Runs** card (year-scoped months Jan→current+1: stage, tested ratio, **Skip run** / **Upload CSV** when no run file, **Open paperwork** when a run exists); **Performance** accordion (month profitability deep-dive); Paperwork entry button |
...
| `frontend/src/features/monthlyRoutes/MonthlyDashboardIssues.tsx` | Dashboard Issues tab — missing ST link / price tables |
| `frontend/src/features/monthlyRoutes/MonthlyDashboardRouteBreakdown.tsx` | Dashboard **Financial Performance** tab — route expense vs revenue breakdown table |
| `frontend/src/features/monthlyRoutes/MonthlyDashboardRoutePerformance.tsx` | Dashboard **Route Performance** tab — route health scorecard table (ST/field duration, pre-route gap, sprawl, monitoring) |
| `frontend/src/features/monthlyRoutes/MonthlyDashboardLocationMetrics.tsx` | Dashboard **Location Metrics** tab — lowest monthly price and lowest price/hr on site |
| `frontend/src/features/monthlyRoutes/routeHealthScorecard.ts` | Client-side health metrics — capacity bands, km/building, monitoring % |
...
**Monthlies dashboard:** ``GET /api/monthly_routes/dashboard`` returns the active route list plus ``current_month_run`` (``workflow_stage`` for the requested calendar month; optional ``month_date`` query, default Pacific current month). Each route row includes ``service_trade_job_dot`` (``green`` when the cached ST testing job is completed; ``green_light`` when released but not completed; ``blue_light`` when scheduled but not released; ``red`` when there is no ST testing job for that month; ``grey`` otherwise) for the **Routes** tab calendar cards. The dashboard at ``/monthlies`` derives KPI counts client-side from the **current month** payload: routes in ``awaiting_office_review`` (to process), scheduled routes still in ``draft`` (to prepare), and open tickets. Calendar route cards are tone-colored: light green when field work is complete (`awaiting_office_review`, `ready_to_close`), dark green when reviewed and closed (`completed`), yellow when the run was skipped (`skipped`), light blue prepared, dark blue field in progress, grey otherwise.

**Bulk ServiceTrade job release (Routes tab):** ``GET /api/monthly_routes/dashboard/st_job_release_status?month_date=YYYY-MM-01`` returns eligible scheduled testing-job counts for bulk release/unrelease (current and future Pacific months only). ``POST /api/monthly_routes/dashboard/st_job_release`` streams NDJSON progress while calling ServiceTrade ``PUT /appointment/{id}`` with ``released: true|false`` on each route's qualifying appointment, then re-syncs ``monthly_route_run_timing_month`` for that route/month. UI: toolbar button **Release all ST jobs** / **Unrelease all ST jobs** (hidden when no eligible jobs or month is in the past). Completed jobs, routes without an ST link, and routes with no scheduled testing job are skipped.

**ST appointment vs route date:** The run-timing sync caches ``service_trade_qualifying_appointment_on`` (Pacific calendar date of the qualifying appointment ``windowStart``). The dashboard compares that to ``effective_route_test_day`` for each scheduled testing job; when they differ, the route card shows a **Date mismatch** pill (tooltip lists both dates).

**Monthlies dashboard Issues tab:** ``GET /api/monthly_routes/dashboard/issues`` returns lists of **active** library locations (excluding R99 training demo stops) missing: ServiceTrade site link, ``price_per_month``, key link, and/or map coordinates (``latitude`` / ``longitude``). A site may appear in multiple lists. Cancelled, on-hold, and other non-active statuses are omitted. UI: ``MonthlyDashboardIssues`` on the **Issues** tab — each row links to ``/monthlies/locations/:id``.

**Monthlies dashboard Financial Performance tab:** KPI tiles (routes to process, runs to prepare, open tickets) plus **route expense breakdown** (``MonthlyDashboardRouteBreakdown``). ``GET /api/monthly_routes/dashboard/route_breakdown`` (default ``range=last_month``) returns all active routes (≥1 active library location; R99 demo excluded) with **building count**, **median avg hours** (ServiceTrade testing-job onsite clock span per Pacific month, median across synced months; values **between 7.5 and 8 hrs bill as 8** for labour expense and display as ``~8`` with true median on hover; **8 hrs and above use the actual median**), **tech count** (``MonthlyRoute.tech_count`` or default 2), **monthly expense**, **avg monthly revenue** (per month: ``price_per_month`` when ``billing_status=bill``, else when billing is unset the legacy tested-outcome sum; ``do_not_bill`` and ``legacy`` contribute \$0; averaged across months with revenue > 0), **monthly net**, and **net %**. Query ``range``: ``last_month`` (previous Pacific calendar month; default), ``last_quarter`` (most recent completed calendar quarter, e.g. Jan–Mar), ``ytd``, or ``last_12_months``. Legacy ``months=N`` still works when ``range`` is omitted and ``months`` is supplied. Response includes ``period_label``, ``period_start``, ``period_end``, ``revenue_columns`` (per-month headers such as ``MAY REVENUE``), ``show_avg_monthly_revenue`` (false for last month), and ``show_total_revenue`` (true for last quarter, year to date, and last 12 months — sum of per-month revenue in the window). Each row includes ``monthly_revenues`` (revenue per month in the window — ``billing_status=bill`` when set, otherwise tested-outcome ``price_per_month`` when billing is unset; with ``revenue_status`` of ``skipped`` only when office staff used **Skip** on the route-month run (``source=office_skip``), or ``no_data`` when there is no revenue) plus ``avg_monthly_revenue`` and ``total_revenue`` when applicable. Month revenue labels match the route detail **Runs** card: no run file → **No data**; office skip run → **Skipped**. Run timing is read from ``monthly_route_run_timing_month`` (populated by ``python -m app.scripts.update_monthly_route_run_timing``). Routes whose **every month in the selected window** was office-skipped (``workflow_stage=skipped``) appear in the main table with **$0 expense** and **$0 revenue** instead of the insufficient section. Cost constants in ``cost_constants``: labour **$45/hr**, truck **$25/mo** (displayed in the UI formula tooltip; admin editing deferred). Routes with no run time samples and no full-period skip (`has_sufficient_run_time_data: false`) appear in a separate **Insufficient run time data** section with a warning badge; net % is not color-coded. CSV export blanks profit columns for those routes. Default sort: route number ascending (R1, R2, …); column headers remain sortable. Net % cells use a continuous red → yellow → green gradient (roughly 40–72% net) when run time data exists. A **Range** toolbar above the table switches the breakdown window. Responses are **cached 30 minutes** server-side; the Financial Performance UI also keeps a **30-minute session cache** per range for instant reload when revisiting the tab. Office sets per-route tech count via ``PATCH /api/monthly_routes/routes/<id>`` with ``tech_count`` (1–9 or null for default) — UI: ``RouteTechCountCard`` on route detail Comments accordion.

**Monthlies dashboard Location Metrics tab:** ``MonthlyDashboardLocationMetrics`` on the **Location Metrics** tab. ``GET /api/monthly_routes/dashboard/location_metrics`` (same ``range`` query params as route breakdown) returns active library sites (R99 demo excluded). **Lowest monthly price** lists the bottom 10 by current ``price_per_month`` (no visit-time requirement). **Lowest price per hour on site** lists the bottom 10 by ``price_per_month`` ÷ avg visit time among sites with at least one sheet or portal **time-in/time-out** in the window. Per-location-month visit minutes are stored in ``monthly_location_visit_timing_month`` (sheet time preferred, else portal stop clocks; one batched portal query per refresh) to keep last-quarter loads fast. The endpoint is not cached — each page load recomputes rankings from current library prices and visit clocks. Optional scheduled warm-up for visit timing rows: ``python -m app.scripts.refresh_location_visit_timing_lookup`` (``MONTHLY_LOCATION_VISIT_TIMING_LOOKBACK``, default 12 Pacific months).

**Monthlies dashboard Route Performance tab:** ``MonthlyDashboardRoutePerformance`` on the **Route Performance** tab — **route health scorecard** (fleet-wide operational table). ``GET /api/monthly_routes/dashboard/route_performance`` (same ``range`` / ``months`` query params as route breakdown) returns all active routes (R99 demo excluded) with **building count**, **distance_meters** and **duration_seconds** from cached ``monthly_route_calculated_path`` (Mapbox; no refresh on dashboard load), **avg_hours** (median ServiceTrade testing-job clock-in to clock-out), **field_avg_hours** (median first-stop time-in to route end — ServiceTrade clock-out when available, else last stop time-out), **pre_route_gap_minutes** (median minutes from ServiceTrade job clock-in to first stop time-in), **skipped_non_annual** (non-annual skips only; count for ``last_month``, **total** for ``last_quarter``, average across months with testing activity for ``ytd`` and ``last_12_months``), **monitoring_site_count** (active stops with company/account/password — structured fields or parsed legacy notes), **tech count**, and **monthly net %** (same formula as Financial Performance for the selected range). The UI derives **capacity band** (under &lt;7 hr, healthy 7–8 hr, full 8–8.5 hr, over &gt;8.5 hr) from **field duration** when available (else ST duration), **sprawl** (km per building from Mapbox path distance), and **monitoring %** client-side. The health table columns are Route, Buildings, **ST duration**, **Field duration** (with capacity band badge), **Pre-route** gap, Sprawl, Monitoring %, Skips (range-aware label), and Net %. Routes with insufficient ServiceTrade run timing appear in a separate **Insufficient run time data** section (sprawl/monitoring/skips still shown; duration and net % need run timing when ST data is missing). CSV export, sortable columns, Range toolbar, and 30-minute server + session cache mirror the financial breakdown tab.
...
| `tests/test_monthly_dashboard_issues_api.py` | `GET /api/monthly_routes/dashboard/issues` — active sites missing ST link, price, key link, or map pin; R99 demo excluded |
| `tests/test_monthly_dashboard_route_earnings_api.py` | `GET /api/monthly_routes/dashboard/route_earnings` — top/bottom revenue ranking, typical end time, R99 excluded |
| `tests/test_monthly_dashboard_route_breakdown_api.py` | `GET /api/monthly_routes/dashboard/route_breakdown` — expense vs avg revenue, tech count, R99 excluded |
| `tests/test_monthly_dashboard_location_metrics_api.py` | `GET /api/monthly_routes/dashboard/location_metrics` — site price/hr ranking, visit time required, R99 excluded |
| `tests/test_monthly_dashboard_route_performance_api.py` | `GET /api/monthly_routes/dashboard/route_performance` — skips, Mapbox cache, monitoring count, net % parity, R99 excluded |
| `tests/test_route_performance_breakdown.py` | `GET /api/monthly_routes/routes/:id/performance_breakdown` — sheet/portal visit times, ST route fallback, expense formula |
| `tests/test_route_run_timing.py` | Run duration / typical end time from `monthly_route_run_timing_month` cache |
| `tests/test_service_trade_route_run_timing.py` | ST testing-job selection and onsite clock span parsing |
| `tests/test_visit_clock_times.py` | Visit clock parse/format/median helpers for metrics |
...
- ``MonthlyRoute.tech_count`` — optional override for Metrics expense breakdown (null = 2 techs). Edited on route detail **Comments** accordion via ``RouteTechCountCard``.

**Route detail Performance deep-dive:** ``GET /api/monthly_routes/routes/<id>/performance_breakdown?month_date=YYYY-MM-01`` (``app/monthly/route_performance_breakdown.py``) returns ``available_months`` (union of route testing history, run files, and ServiceTrade timing months — including timing rows that have not synced to ``ok`` yet), a month summary (tested revenue, expense, net, route hours, **ServiceTrade run clock-in/clock-out** when synced, revenue per route hour), per-stop rows in worksheet order, and server-computed ``insights``. UI: ``RoutePerformanceBreakdown`` on the route detail **Performance** accordion. **Revenue** on each stop and in month totals counts ``price_per_month`` when ``billing_status`` is ``bill`` (regardless of tested/skipped outcome). When ``billing_status`` is unset, revenue follows the test outcome: **tested** counts; **skipped** (annual or non-annual) does not. ``do_not_bill`` and ``legacy`` always contribute \$0.

**Per-stop visit duration** (priority order):

1. ``MonthlyLocationMonth.sheet_time_in_raw`` / ``sheet_time_out_raw`` (master sheet or route CSV)
2. ``MonthlyStopClockEvent`` pairs on that month row (portal clock-in/out; first open in → last closed out)
3. No per-stop time — stop duration stays blank

Ambiguous ``h:mm`` values without AM/PM infer meridiem for field-route visits: **7–11 → AM**, **12 → PM (noon)**, **1–6 → PM** (``12:41`` to ``1:04`` → 23 minutes, not a midnight wrap). Explicit ``13:00``–``23:59`` still parse as 24-hour clock.
...
**Annual schedule check (office prep + location detail):** The UI calls ``GET /api/monthly_routes/routes/<id>/runs/annual_schedule_check?month_date=YYYY-MM-01`` (live sync on each open; ``cache_bust=1`` or ``sync=1`` bypasses the short dedup window). Location detail uses the upcoming test month with ``&sync=1`` for the testing-history grid. The location detail hero **Annual** field uses ``GET /api/monthly_routes/library/<id>/saved_annual_month?sync=1`` — a per-site ServiceTrade lookup across a rolling ~24-month window (not limited to the current calendar month). Each qualifying inspection job contributes the calendar month of its **earliest** qualifying **released** appointment (not the latest, and not route test-day spanning logic). Jobs with no released appointments are ignored. Live job discovery uses ``scheduledDateFrom/To`` with ``status=scheduled,completed`` (``scheduleDateFrom/To`` alone often returns no site-level inspection jobs).

| Prep row | Condition |
|----------|-----------|
| Orange annual row | Cached ``st_annual_skip_recommended`` for this month **and** no ``annual_test_override`` on the stop |
| Pill “Unreleased annual” | Cached ``st_has_unreleased_annual_in_month`` — qualifying inspection job in the month with no released appointments |
...
When a spanning inspection job touches two calendar months and the location already has an **explicit annual skip** recorded on paperwork for one of those months (``skip_category: annual`` or ``skip_reason`` ``annual`` / ``annual_booked``), ServiceTrade annual auto flags are cleared for the other month(s) of that same spanning job (return visits do not re-trigger automatic annual treatment).
...
- **Office prep / library sync:** Snapshot fields stay aligned during open prep: prep worksheet PATCH mirrors to library master (latest month), and library testing-site PATCH mirrors back into open prep ``MonthlyTestingSiteMonth`` rows. Prep-only fields (`run_comments`, `office_job_comment`, `office_attention`) stay on the stop-month row only. Paperwork **Preparation** view exposes **Skip route** (draft or prepared runs, or when no run file exists; `POST …/runs/skip`) and **Regenerate paperwork** (`POST …/runs/regenerate_prep_stops`), which rebuilds the route-month stop list from the active library route (adds new sites, removes cancelled or unassigned sites, applies library stop order) and clears all run-scoped worksheet progress on remaining stops—the same scope as **Reset run** (outcomes, times, comments, billing, audit change log; legacy billing preserved)—without unprepare or clearing the pre-run message. Blocked after field work starts or when the run is completed. **Draft prep only** (not Ready): each active site row has **Skip** / **Unskip** (`POST` / `DELETE …/worksheet/locations/<id>/prep_skip?month=`) with skip category + office job comment; sets `test_outcome=skipped`, copies the reason into `office_job_comment`, leaves billing unset, and pre-fills the technician portal skip (techs may change or clear it). Row/nav styling: yellow skip only after office **Skip** (``test_outcome=skipped``); legacy sheet ``result_status`` alone does not count. **Orange annual** (prep row + technician portal nav/header) only when the site's annual month matches the run month **and** ServiceTrade has a qualifying inspection appointment in that month (``scheduled_annual_auto_skip`` on worksheet stops; prep uses the same ServiceTrade check). Mismatch or missing ST job: no orange highlight and legacy annual skips do not pre-color as skip on the portal.
...
**Read-only runs:** `MonthlyRouteRun.source = csv_import` → `portal_read_only` when field work has ended or the run is office-completed (historical spreadsheet imports for current/prior months auto-close on upload). Future-month prep CSV imports stay editable in the technician portal once the run is started. `office_skip` runs are always portal read-only. Office Paperwork **run review** may still edit billing and test outcomes when the run is reopened and field work has ended.
...
**Site test history (worksheet hero):** On an active run, each stop hero card shows a **History** button in the hero actions row. It opens a modal with month navigation and frozen field submission history (same data as billing **exact history**). On **phone and iPad** (viewport ≤1024px), history renders as vertically stacked field cards (`PortalSiteHistoryFieldCards`); on wider screens it uses `OfficeWorksheetReadOnlyTable`. The modal opens on the **most recent month** with a captured field submission (`field_ended_at`); **Previous month** / **Next month** step through consecutive calendar months (forward bound: Pacific current month). Empty states match billing when no route or no submission exists for that month.
...
**Run summary (worksheet only):** After a successful `POST …/runs/end`, the worksheet shows a success modal (`PortalRunSummaryModal.tsx`) with an animated run summary. The server builds `run_summary` in `app/monthly/portal_run_summary.py` and returns it on the end-run response (not on idempotent re-end). Metrics:

- **Stop outcomes** — tested, annual skips, and other skips for the run month.
- **Field duration** — first stop time-in through field end (portal clocks; `field_ended_at` when set).
- **Finish time** — Pacific clock time when field work ended.
- **Comparisons** (when at least two prior months of history exist on this route, last 12 months excluding the current month) — field duration and finish time vs this route’s medians, **annual-adjusted**: each annual skip subtracts an estimated visit duration (median historical annual-skip visit time on the route when ≥ 3 samples, else 12 minutes) before comparing. Comparison lines use ±5 minutes as “on time.”

The UI count-up animates outcome counts and field duration, then reveals comparison lines (respects `prefers-reduced-motion`). Implementation: `portalRunSummary.ts`, `useCountUp.ts`, `usePortalWorksheet` (`runSummaryModal` state).

**End-of-run debrief (worksheet wizard step 2):** After **Next** on the performance step, the same modal (`PortalRunSummaryModal.tsx`) shows an optional rich-text **End of run summary** (bold + colors, same sanitizer as stop job comments). **Skip** closes without saving; **Save debrief** calls ``PATCH /api/technician_portal/routes/:id/runs/field_end_summary`` with ``{ field_end_summary }`` (empty clears to null). Stored on ``MonthlyRouteRun.field_end_summary`` (migration ``z29a1b2c3d4e9``). Preserved when technicians **reopen field**; cleared on **Reset run**; pre-filled on the next end run so the tech can edit. Office can edit the same field during **Run review** on paperwork via ``PATCH /api/monthly_routes/routes/:id/runs`` with ``{ month_date, field_end_summary }`` (`RunDetailsFieldEndSummaryCard.tsx` on ``MonthlyRoutePaperworkPage``); blocked before field end and after the job is completed/reopened to exact history. **Exact history** shows the summary read-only above the frozen submission table.
...
Body: `{ "billing_status": "bill" | "do_not_bill" | "unset" }`. Allowed only after technicians **end field** on the run (`office_may_edit_billing`; `409` + `code: billing_before_field_end` while field work is open). Rejects when the row is `legacy` (`code: billing_legacy_locked`). CSV-import runs use the same office review gates (technician portal remains read-only via `portal_read_only`). **Legacy outcomes** (`result_status` without `test_outcome`, e.g. CSV import) show in the run-review outcome dropdown as All good or Skip and may be changed or cleared like portal outcomes once field work has ended. **Skipped (annual)** is an extra office-only dropdown value that sets `skip_category: annual` (orange annual cell); generic **Skip** still opens the skip-reason modal. When a stop is **skipped** with an explicit ``skip_category`` or ``skip_reason`` (other than annual / ``annual_booked``), paperwork and run review show that reason instead of inferring **Skipped (annual)** from the site's annual month alone. **Auto set billing** (run review button) sets **Bill** when any testing site at the location is all good, passed with problems, or legacy tested; sets **Do not bill** when every site at the location is an annual skip / annual month. Recording a portal or office **Skipped (annual)** outcome also auto-defaults billing to **Do not bill** (same as **Bill** on all good). Other outcomes (failed, non-annual skip, pending) are left unchanged; legacy billing rows are skipped. **Review complete** only auto-fills rows still **unset**; an office **Bill** or **Do not bill** choice (including Bill on a skipped stop) is preserved.
...
**Paperwork UI:** Single locked view per run phase (see §5.2). **Run preparation** (before `started_at`, including the **Ready** step after mark prepared) uses the same compact worksheet layout as **Exact history** (wide page, `tw-office-*` table): columns are `#` (with drag reorder), Address, Access, Panel, Monitoring, Deficiencies, Testing procedures, Location comments, and **Office comments** (`office_job_comment`; no Result column). All prep fields remain inline-editable; drag reorder uses `PUT …/location_order`. Non-empty office comments highlight the prep row purple and appear read-only in the technician portal. `GET …/run_details` includes `panel` and `panel_location` on each location for the prep Panel column. **Run review** shows live outcomes, billing, tickets, and deficiencies; field values use the same rich-text rendering as prep (technician edits are not highlighted separately). **Exact history** shows the frozen technician submission from `GET .../run_details/field_submission` (empty state when no snapshot exists). **Field changes** is not a Paperwork tab in this iteration. **Complete** (after field end) requires every billing location to be `bill` or `do_not_bill` (not `unset`); the same action stamps office review and closes the job.
