...
**Refresh:** `mergeWorkflowQueueIntoPayload` overlays the workflow queue on GET/SSE merges so stale server data cannot reopen a site the technician already clocked out of. When a fetch shows an office **Reset run** (cleared run header and/or bulk cleared stop outcomes), `serverRunWasExternallyReset` purges the worksheet cache and all offline queues for that route-month so skipped sites do not reappear from `localStorage`.
